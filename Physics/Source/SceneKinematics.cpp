...
void SceneKinematics::Update(double dt)
{
	...
	if(Application::IsKeyPressed(VK_SPACE) || Application::IsMousePressed(0))
	{
		...
		for (int i = 0; i < m_goList.size(); i++)
		{
			if (!m_goList[i]->active)
			{
				...
				m_goList[i]->type = GameObject::GO_BALL;
				m_goList[i]->pos = Vector3(Math::RandFloatMinMax(0, m_worldWidth), Math::RandFloatMinMax(0, m_worldHeight), 0);
				m_goList[i]->vel = Vector3(Math::RandFloatMinMax(-20, 20), Math::RandFloatMinMax(-20, 20), 0);
				break;
			}
		}
	}
	if(Application::IsKeyPressed('V'))
	{
		...
		for (int i = 0; i < m_goList.size(); i++)
		{
			...
			{
				m_goList[i]->active = true;
				m_goList[i]->type = GameObject::GO_CUBE;
				m_goList[i]->pos = Vector3(Math::RandFloatMinMax(0, m_worldWidth), Math::RandFloatMinMax(0, m_worldHeight), 0);
				break;
			}
		}
	...
	cursorResult = m_ghost->pos - Vector3(x / w * m_worldWidth, (h - y) / h * m_worldHeight, 0);
	...
	else if(bLButtonState && !Application::IsMousePressed(0))
	{
		...
		for (int i = 0; i < m_goList.size(); i++)
		{
			if (!m_goList[i]->active)
			{
				m_goList[i]->active = true;
				m_goList[i]->type = GameObject::GO_BALL;
				m_goList[i]->pos = Vector3(x / w * m_worldWidth, (h - y) / h * m_worldHeight, 0);
				m_goList[i]->vel = Vector3(cursorResult);
				m_timeGO = m_goList[i];
				break;
			}
		}
		...
		m_timeEstimated1 = cursorResult.y / -m_gravity.y;
		m_timeTaken1 = 0;

		m_heightEstimated = (0.5 * -m_gravity.y * m_timeEstimated1 * m_timeEstimated1) + m_timeGO->pos.y;
		m_heightMax = m_timeGO->pos.y;

		m_timeEstimated2 = sqrt(m_heightEstimated / (0.5 * -m_gravity.y)) + m_timeEstimated1;
		m_timeTaken2 = 0;
	...
	for(std::vector<GameObject *>::iterator it = m_goList.begin(); it != m_goList.end(); ++it)
	{
		...
		if(go->active)
		{
			if(go->type == GameObject::GO_BALL)
			{
				...
				go->vel += m_gravity * dt * m_speed; //DeltaV(dv) change in velocity
				go->pos += go->vel * dt * m_speed;   //DeltaS(ds) change in position
				...
				for (std::vector<GameObject *>::iterator it2 = m_goList.begin(); it2 != m_goList.end(); ++it2)
				{
					...
					if (go2->active == true && go2->type == GameObject::GO_CUBE)
					{
						if ((go->pos - go2->pos).LengthSquared() < ((go->scale.x + sqrt(go2->scale.x * go2->scale.x) +  (go2->scale.y * go2->scale.y))))
						{
							go->active = false;
							go2->active = false;
							break;
			...
			if(go->pos.x < 0 || go->pos.x > m_worldWidth || go->pos.y < 0 || go->pos.y > m_worldWidth)
			{
				go->active = false;
			}
		}
...
void SceneKinematics::Render()
{
	...
	ss.str("");
	ss << "Estimated Time 1:" << m_timeEstimated1;
	...
	ss.str("");
	ss << "Estimated Time 2:" << m_timeEstimated2;
	...
	{
		ss.str("");
		ss << "Estimated HD:" << m_timeGO->vel.x * m_timeEstimated2;
		RenderTextOnScreen(meshList[GEO_TEXT], ss.str(), Color(0, 1, 0), 3, 0, 33);
		ss.str("");
		ss << "Actual HD:" << m_timeGO->vel.x * m_timeTaken2;
		RenderTextOnScreen(meshList[GEO_TEXT], ss.str(), Color(0, 1, 0), 3, 0, 36);
	}
